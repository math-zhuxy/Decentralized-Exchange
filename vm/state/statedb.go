...
import (
	...
	"blockEmulator/vm/trie/triestate"
	"blockEmulator/vm/trie/utils"
...
func New2(root common.Hash, db Database) (*StateDB, error) {


	tr, err := db.OpenTrie(root)
	if err != nil {
		fmt.Println("tr, err := db.OpenTrie(root)错误"+err.Error())
		return nil, err
	}
	reader, err := db.Reader(root)
	if err != nil {
		fmt.Println("reader, err := db.Reader(root)错误"+err.Error())
		return nil, err
	}
	sdb := &StateDB{
		db:                   db,
...
func New(root common.Hash, db Database) (*StateDB, error,string) {
	global3.Lock.Lock()
	defer global3.Lock.Unlock()

	if global3.GlobalStateDB != nil {
		return global3.GlobalStateDB.(*StateDB), nil,"old"
	}
...
func (s *StateDB) AddRefund(gas uint64) {
	s.journal.refundChange(s.refund,gas)
	s.refund += gas
}
...
func (s *StateDB) getOrNewStateObject(addr common.Address) *stateObject {
	...
	if obj == nil {
		...
		//s.AddBalance(addr, uint256.MustFromBig(setString), tracing.BalanceInilizeByXBZ)
	}
	return obj
}
...
func stringToFixedByteArray(s string) [32]byte {
	var b [32]byte
	copy(b[:], []byte(s))
	for i := len(s); i < 32; i++ {
		b[i] = 0
...
func (s *StateDB) RevertToSnapshot(revid int) {
	fmt.Println("执行回滚操作。。。，revid=",revid,"")
