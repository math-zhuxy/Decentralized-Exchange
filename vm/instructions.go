...
func opKeccak256(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if evm.Config.EnablePreimageRecording {
		evm.StateDB.AddPreimage(interpreter.hasherBuf, data) //访问状态树
	}
...
func gasOpbalanceAccountCheck(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
...
// 读取某个账户地址的余额
// 参数 pc: 指向程序计数器的指针
// 参数 interpreter： 指向 EVM 解释器的指针
// 参数 scope： 指向当前执行上下文环境的指针
func opBalance(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	//获得栈顶元素
	slot := scope.Stack.peek()
	//将栈顶元素转换为 20个字节表示的账户地址
	address := common.Address(slot.Bytes20())
	//将20个字节表示的账户地址转换为16进制的字符串
	toString := hex.EncodeToString(address[:])
	//存储跨片读取的最终结果的变量
	var res *uint256.Int = uint256.NewInt(0)

	if Get_PartitionMap(toString) != global.ShardID {
		//如果要进行读操作的账户位于另一个分片，则进行跨片读取
		//创建跨片读请求的对象
		...
		//将跨片读请求的对象使用 json 序列化得到跨片读请求的对象的字节切片表示
		b, _ := json.Marshal(msg)
		//在跨片读请求的对象的字节切片表示的前面添加消息头
		b1 := message.MergeMessage(message.COpbalanceReq, b)
		//向目标分片的 PBFT Leader节点发送跨分片读请求
		go networks.TcpDial(b1, global.Ip_nodeTable[Get_PartitionMap(toString)][0])
		//用于存储跨片读取结果的临时变量
		var balanceres uint64
		//用于判断目标分片是否返回了响应结果
		var ok bool
		//得到当前时间
		now := time.Now()
		//死循环等待目标分片的 PBFT Leader节点返回读请求对应的相应结果
		for {
			...
			balanceres, ok = global.OpBalanceResMap[toString]
			//解锁
			...
			if ok {
				...
				delete(global.OpBalanceResMap, toString)
				//解锁
				global.OpBalanceResMapLock.Unlock()
				//读取成功,跳出循环
				break
		...
		//将跨片读取到的临时变量设置到 存储最终结果的的变量中
		res = uint256.NewInt(balanceres)
	} else {
		//如果要进行读操作的账户位于当前分片，则从本地的状态树中读取 balance
		res = interpreter.evm.StateDB.GetBalance(address)
	}
	//将最终的结果设置到栈顶元素
	slot.Set(res)
	//返回结果为空，因为最终的结果已经存储在栈顶，无需在此处作为函数返回值来返回
	//返回 error为空，因为跨片读取一定不会出错，即使是读取超时也算是成功的情况
	return nil, nil
}
...
func opExtCodeSize(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if witness := interpreter.evm.StateDB.Witness(); witness != nil { //访问状态树
...
func opExtCodeCopy(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	code := interpreter.evm.StateDB.GetCode(addr) //访问状态树
...
func opExtCodeHash(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	slot := scope.Stack.peek()
	address := common.Address(slot.Bytes20())
	if interpreter.evm.StateDB.Empty(address) { //访问状态树
		slot.Clear()
	} else {
...
func opCreate(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	res, addr, returnGas, suberr := interpreter.evm.Create(scope.Contract, input, gas, &value,scope.Contract.UUID)
...
func opCreate2(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	res, addr, returnGas, suberr := interpreter.evm.Create2(scope.Contract, input, gas,
		&endowment, &salt,scope.Contract.UUID)
...
func opCall(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	// Get the arguments from the memory. 从memory中取出调用参数
	...
	ret, returnGas, err := interpreter.evm.OpCall(scope.Contract, toAddr, args, gas, &value)
