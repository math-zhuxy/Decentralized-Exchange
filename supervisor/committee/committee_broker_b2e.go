...
func data2tx(data []string, nonce uint64) (*core.Transaction, bool) {
	if data[6] == "0" && data[7] == "0" && len(data[3]) > 16 && len(data[4]) > 16 && data[3] != data[4] {
		...
		gasPrice, _ := new(big.Int).SetString(data[10], 10)
		gasUsed, _ := new(big.Int).SetString(data[11], 10)
...
func (bcm *BrokerCommitteeMod_b2e) MsgSendingControl() {
	...
	//
	//		mytool.Mutex.Lock()
	//		if len(mytool.AutoGeneratedB2EQueue) == 0 {
	//			mytool.Mutex.Unlock()
	//			continue
	//		}
	//
	//		queueCopy := make([]*core.Transaction, len(mytool.AutoGeneratedB2EQueue))
	//		copy(queueCopy, mytool.AutoGeneratedB2EQueue)
	//		mytool.AutoGeneratedB2EQueue = mytool.AutoGeneratedB2EQueue[:0]
	//
	//		mytool.Mutex.Unlock()
	//
	//		//bcm.BrokerModuleLock.Lock()
	//		itx := bcm.dealTxByBroker(queueCopy)
	//		//bcm.BrokerModuleLock.Unlock()
	//		bcm.txSending(itx)
	//
	//	}
...
//	println("consume time(sec.) ", time.Now().Sub(now).Seconds())
...
func (bcm *BrokerCommitteeMod_b2e) dealTxByBroker(txs []*core.Transaction) (itxs []*core.Transaction) {
	...
	if len(brokerRawMegs) > 1000 {
		//a := make([]*message.BrokerRawMeg, 0)
		//b := make([]*message.BrokerRawMeg, 0)
		//for i := 0; i < len(brokerRawMegs); i++ {
		//	if brokerRawMegs[i].Tx.Isimportant {
		//		a = append(a, brokerRawMegs[i])
		//	} else {
		//		b = append(b, brokerRawMegs[i])
		//	}
		//}
		//if len(b) > 1000 {
		//	b = b[:1000]
		//}
		//brokerRawMegs = append(a, b...)
	...
	now := time.Now()
	alloctedBrokerRawMegs, restBrokerRawMeg := Broker2Earn.B2E(brokerRawMegs, bcm.Broker.BrokerBalance)
	println("consume time(millsec.) ", time.Now().Sub(now).Milliseconds())
	for _, item := range restBrokerRawMeg {
		bcm.restBrokerRawMegPool = append(bcm.restBrokerRawMegPool, item)
	}
	//println("2brokerSize ", len(alloctedBrokerRawMegs))
