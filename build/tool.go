...
func GenerateShellFile(nodenum, shardnum, modID int) {
	...
	for j := 0; j < shardnum; j++ {
		for i := 1; i < nodenum; i++ {
			str := fmt.Sprintf("./blockEmulator_Linux_Precompile -n %d -N %d -s %d -S %d -m %d &\n\n", i, nodenum, j, shardnum, modID)
			ofile.WriteString(str)
		}
	}
	str := fmt.Sprintf("./blockEmulator_Linux_Precompile -c -N %d -S %d -m %d &\n\n", nodenum, shardnum, modID)

	ofile.WriteString(str)
	for j := 0; j < shardnum; j++ {
		str := fmt.Sprintf("./blockEmulator_Linux_Precompile -n 0 -N %d -s %d -S %d -m %d &\n\n", nodenum, j, shardnum, modID)
		ofile.WriteString(str)
	}
}
